...
from backend.app.infrastructure.persistence.chat_models import Chat, Message
...
async def update_chat_title_object(db: AsyncSession, chat: Chat, title: str) -> Chat:
    chat.title = title
    await db.commit()
    await db.refresh(chat)
    return chat
